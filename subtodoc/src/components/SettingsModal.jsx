...
const AI_PROVIDERS = [
  { id: 'groq',     label: 'Groq',   model: 'llama-4-scout',      badge: '기본',     keyField: 'groqApiKey',   placeholder: 'gsk_...',  link: 'https://console.groq.com',                 linkLabel: '무료 키 발급 →' },
  { id: 'groq-oss', label: 'OpenAI', model: 'gpt-oss-120b',       badge: 'via Groq', keyField: 'groqApiKey',   placeholder: 'gsk_...',  link: 'https://console.groq.com',                 linkLabel: 'Groq 키 발급 →' },
  { id: 'gemini',   label: 'Gemini', model: 'gemini-2.5-flash-lite', badge: '',      keyField: 'geminiApiKey', placeholder: 'AIza...', link: 'https://aistudio.google.com/app/apikey',   linkLabel: '무료 키 발급 →' },
]
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 overflow-y-auto py-8">
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[2px] p-6 w-full max-w-md space-y-5" style={{ boxShadow: 'rgba(0,0,0,0.3) 0px 0px 5px 0px' }}>
        <h2 className="text-lg font-bold text-[var(--text)] tracking-tight">설정</h2>

        {/* 자막 소스 */}
        <section className="space-y-3">
          <h3 className="text-xs font-bold uppercase tracking-wider text-[#76b900]">자막 소스</h3>
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] mb-2">자막 가져오기 방법</label>
                ...
                <button key={p.id}
                  ...
                  className={`text-left px-3 py-2 rounded-[2px] text-sm font-bold transition-colors ${
                    local.transcriptProvider === p.id
                      ? 'bg-[#76b900]/10 border-2 border-[#76b900] text-[var(--text)]'
                      : 'bg-[var(--bg)] border border-[var(--border)] text-[var(--text-secondary)] hover:border-[#76b900]/60 hover:text-[var(--text)]'
                  }`}
                >{p.label}</button>
              ))}
            </div>
          ...
          {local.transcriptProvider === 'supadata' && (
            <div>
              ...
              <input type="password" value={local.supadadataApiKey}
                ...
                placeholder="sup_..." />
              <p className="text-xs text-[var(--text-muted)] mt-1">무료 플랜: 10회/일. supadata.ai에서 가입 후 API 키 발급</p>
            </div>
          )}
          {local.transcriptProvider === 'local' && (
            <div>
              ...
              <input type="text" value={local.localServerUrl}
        ...
        <section className="space-y-3">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] mb-2">AI 모델 선택</label>
            <div className="flex flex-col gap-1.5">
              {AI_PROVIDERS.map(p => {
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-normal ${isSelected ? 'text-[var(--text-secondary)]' : 'text-[var(--text-muted)]'}`}>{p.model}</span>
                    </span>
                    {p.badge && (
                      <span className={`text-xs px-1.5 py-0.5 rounded-[2px] font-bold ${isSelected ? 'bg-[#76b900]/30 text-[#76b900]' : 'bg-[var(--surface)] text-[var(--text-muted)]'}`}>{p.badge}</span>
                    )}
                  </button>
                )
              })}
            ...
            {(local.provider === 'groq' || local.provider === 'groq-oss') && (
              <p className="text-xs text-[var(--text-muted)] mt-1.5">Groq · OpenAI(gpt-oss) 모두 같은 Groq API 키를 사용합니다.</p>
            )}
          </div>
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] mb-1">
              {currentProvider.label} API Key
              <a href={currentProvider.link} target="_blank" rel="noopener noreferrer" className="ml-2 normal-case font-normal text-[#76b900] hover:text-[#bff230] text-xs">{currentProvider.linkLabel}</a>
            </label>
            <input key={currentProvider.keyField} type="password"
          ...
          <div>
            ...
            <select value={local.language} onChange={e => setLocal(l => ({ ...l, language: e.target.value }))}
              className="w-full px-3 py-2 bg-[var(--bg)] border border-[var(--border)] rounded-[2px] text-sm focus:outline-none focus:border-[#76b900] text-[var(--text)]">
